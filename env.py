...
class Sculpt:
    ...
    def reset(self, test=False, begin_num=False):
        ...
        self.gt = torch.zeros([self.batch_size, 1, width, width, width], dtype=torch.uint8).to(device)
    ...
    def step(self, action):
        self.canvas = decode(action, config.path['root'], 128, self.batch_size)
        self.stepnum += 1
        ob = self.observation()
