...
####elements potentially to remove
...
### don't know what this should be a thing
canvas_cnt = args.divide * args.divide


T = torch.ones([1, 1, width, width, width], dtype=torch.float32).to(device)
#### I need to look at how this is used
img = cv2.imread(args.img, cv2.IMREAD_COLOR)
origin_shape = (img.shape[1], img.shape[0])

coord = torch.zeros([1, 3, 128, 128])
for i in range(128):
    for j in range(128):
        for k in range(128):
...
#### 
actor = Actor(128) # action_bundle = 5, 65 = 5 * 13
actor.load_state_dict(torch.load(config.test['actor']))
actor = actor.to(device).eval()
#Decoder = Decoder.to(device).eval()

canvas = torch.zeros([1, 1, width, width, width]).to(device)
...
#### test process to sort
with torch.no_grad():
    if args.divide != 1:
        config.test['max_step'] = config.test['max_step'] // 2
    for i in range(config.test['max_step']):
        stepnum = T * i / config.test['max_step']
        actions = actor(torch.cat([canvas, img, stepnum, coord], 1))
        canvas, res = decode(actions, canvas)
        print('canvas step {}, L2Loss = {}'.format(i, ((canvas - img) ** 2).mean()))
        for j in range(5):
            save_img(res[j], config.test['imgid'])
            config.test['imgid'] += 1
    # if args.divide != 1:
